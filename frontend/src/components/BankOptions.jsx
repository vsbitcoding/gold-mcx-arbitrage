    ...
    <section className="bo-page" aria-labelledby="bo-title">
      ...
      {view === "live" ? (
        <div id="bo-live-panel" className="bo-panel" role="tabpanel" aria-labelledby="bo-live-tab">
          ...
          {board?.status?.message && !board.status.ready && <div className="bo-alert">{board.status.message}</div>}
          <div className="bo-table-meta"><span className="bo-strike-guide"><strong>15 strikes</strong><span>7 below · ATM · 7 above</span><span>500-point steps</span>{board?.status?.message && board.status.ready && <span className="bo-status-note">{board.status.message}</span>}</span><span><span className="bo-freshness-dot is-fresh" /> Fresh <span className="bo-freshness-dot" /> Awaiting / stale <span className="bo-quality-key" aria-hidden="true">i</span> Quote details</span></div>
